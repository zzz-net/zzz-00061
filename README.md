...
> ⚠️ **真实服务端口为 5002**，不是默认的 5000。确认进程启动成功后请访问此地址。
...
- 紧急程度标签（普通/紧急/特急）
- 升级标签、冷却标签
- **命中原因（新增）**：明确列出每个条件的匹配情况，如：
  - `issue_types: 期望值['材料缺页', '标签不清'], 实际值材料缺页，匹配成功`
  - `statuses: 期望值['OPEN', 'IN_PROGRESS'], 实际值OPEN，匹配成功`
  - `min_hours_open: 期望值24, 实际值36.5，匹配成功`
- **冲突检测**：如果同时命中多条策略，显示橙色警告条，列出所有命中的策略名称和优先级
- **裁决结果（增强）**：显示最终选择的策略和裁决理由，包括完整排序：
  - `存在3个匹配策略，按优先级排序: 高优先级策略(优先级100) → 默认策略(优先级10) → 低优先级策略(优先级5)`
  - `冲突裁决：选择优先级最高的策略"高优先级策略"(优先级=100)`
- **未选中的高优先级规则（新增）**：列出优先级≥选中策略但未命中的规则及原因，如：
  - `策略"特急催办"(优先级=200): 优先级(200) ≥ 选中策略优先级(100)，但未满足触发条件: 问题类型不匹配: 期望['特急问题'], 实际材料缺页`
- **未命中策略列表（新增）**：所有未命中的策略及其失败原因，便于调整规则
...
>
> 🔍 **预演误判防护**：所有条件检查都返回详细的期望值vs实际值对比，避免因条件配置错误导致的误判。只有**全部触发条件都满足**的规则才算命中。
